...
Status: consolidated after the DMK translation exercise (`translations/`) and a first implementation pass (`proto/` — a Rust interpreter + player whose conformance suite runs the entire translation corpus verbatim, production boss card included). Findings F1–F20 and the adopted conventions are folded in here; NOTES.md/SCANNED.md remain as the working record. Sections marked **[decide]** are open decisions.
