...
// editor-src/maku-codemirror.js
var FORM_WORDS = new Set([
  "def",
  "defn",
  "defmacro",
  "defpattern",
...
var makuLanguage = StreamLanguage.define({
  name: "maku",
  ...
  token(stream, state) {
    ...
    if (/^-?(?:\d+\.\d+|\d+|\.\d+)(?:[eE][+-]?\d+)?$/.test(tok))
      return "number";
    if (tok.startsWith(":"))
      return "keyword";
    if (tok.startsWith("$"))
      return "variableName.special";
    if (FORM_WORDS.has(tok))
...
var makuHighlight = HighlightStyle.define([
...
var singleLineTheme = EditorView.theme({
  "&": {
    minHeight: "120px"
  },
  ".cm-content": {
...
function createMakuEditor({ parent, value = "", onChange = () => {}, compact = false }) {
  const theme2 = new Compartment;
  const view = new EditorView({
    parent,
    state: EditorState.create({
      doc: value,
      extensions: makuExtensions(onChange, [theme2.of(compact ? singleLineTheme : [])])
    })
  });
  return {
    view,
