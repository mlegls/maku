...
Dyn values MUST be callable in application position — `(d 3.5)` samples at epoch-local t = 3.5, and curve dyns take the curve parameter as a second argument `(d t u)`. A plain `(fn [t] ...)` MUST be accepted anywhere a dyn is expected.
*Why:* dyn<T> ≅ t -> T; application-as-sampling replaces a `sample` builtin. Rationale: `docs/notes/evolve-design.md`.
