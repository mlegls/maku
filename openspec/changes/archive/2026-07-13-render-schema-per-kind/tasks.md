...
- [x] 4.1 Touhou lib declares its kinds (`:sprite` over point geometry with family/color/variant/scale; the beam polyline kind); render-touhou negotiates for them and reads declared schemas instead of probe-and-default (keep ignore/default for undeclared kinds).

## 5. Gates and sync

- [x] 5.1 Tests: per-kind scoping (same key, two kinds), declared-kind load fixing + new-key error, manifest failure/lint paths, adapter end-to-end (imported kind renders as local), batch/row equivalence under kinds. Full gate: `cargo test --release --manifest-path crates/core/Cargo.toml` + the 4 ignored oracle card suites. Commit each coherent change-set.
