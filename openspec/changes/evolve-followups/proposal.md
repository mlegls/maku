...
The `remat`/`change-col` contract is settled and landed (write queue, functional `change-col` composition, partial `(remat h spec-map)`, per-slot epochs, sited/live evolves, slew/smooth as prelude macros; semantics in `docs/language.md` and `docs/notes/evolve-design.md`), but the track has open follow-ups.
...
- Governing notes: `docs/notes/evolve-design.md`, `openspec/changes/model-split/design.md`.
