...
- [x] 1.1 Organize `proto/mesh-touhou` around a genre-facing pack, immutable `TouhouProfile`, schema bindings, shared output arena, resource registry, and internal sprite/ribbon processors without extracting a generic crate.
...
- [x] 3.1 Update `proto/player` to resolve the shared cold resource/material manifest and submit the pack's ordered instance/indexed commands; isolate macroquad point-instance expansion or chunking in the adapter.
- [x] 3.2 Turn the existing `proto/web` directory into a workspace Rust host crate depending on core and `mesh-touhou`, and update `proto/web/build.sh` to build that crate.
