...
Landed 2026-08 (`entity-representation-flip`): rows dropped to
`spec_id` + captures + `RowFrame` pose + `SpawnAxis`; culled rows
detach from carrier lookup at cull and release the spec ref at slot
reuse, preserving generation-valid dead-row handle reads.
