...
The set of `(from-host :name)` sites in the loaded card SHALL be checked against the channels the host provides at load time. A required host channel the host does not provide SHALL fail the load with an error naming the channel â never mid-run.
