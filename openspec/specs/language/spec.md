...
## Purpose
The Maku language's internal specification: value universe, the
Closed/Scanned signal model, frames, broadcasting, entities/fields,
channels, and the action layer. Requirements are the current-truth
anchor; the Reference section carries the full design-spec text.
## Requirements
...
Founding conclusions (from the original design essay; full text in git
history as docs/design.md):

- Bullets cannot be only pure functions of t: homing/reactive behavior is
  an ODE over an unpredictable input signal (stepwise integration is the
  only general evaluation), events mutate bullets (function splicing
  reintroduces state as closure environments), and designers author
  velocities that rarely integrate symbolically. Hence the two-constructor
  signal model with rematerialization as the sanctioned exit.
- The field model `f(t, pos) -> bullet?` fails as core representation:
  rendering, grazing, cancellation, and scoring need enumeration and
  identity; the efficient field implementation IS an explicit bullet list.
  The field survives only as the collision-query interface.
- The game is a function of time AND input history; input arrives
...
- Make an operation intrinsic only when it is hard to implement well in
  ...
  `match` and seq views. The candidate array/control verbs and the
  K-inspired direction are profiling-driven, not vocabulary-driven.
- Deterministic math/matrix intrinsics are language semantics: native and
  wasm must replay identically; dependency upgrades must not silently
  change behavior. Smooth noise should be a pure function of coords+seed.
- Stdlib stance: Touhou/DMK conventions live in `crates/core/lib/touhou.maku`;
  core stays a 2D graphing + collision/rule/render-row engine; ergonomic
  row-wise APIs are lib/prelude sugar, never core special forms.
...
- `EntityRef` — stable handle with generation. Returned by `spawn`, consumed by `manip`, and safe across row reuse.
...
This is the semantic model. The runtime representation is a row in dense SoA storage: dyn programs/function pointers, typed meta matrices, sampled pose buffers, collider work buffers, and cold collider projector data. Render rows are emitted by rules into the per-tick render buffer. Per-entity hot state draws only from fixed typed storage so scanned state packs into columns and steps vectorize pool-at-a-time. Control-layer values may be richer, but anything retained across ticks must lower to the fixed runtime universe.

### The type discipline: Signal / Function / Action

`Signal a` is **not a function type** — a first-class time-varying value, composed pointwise and sampled (`snap`), never applied. *Both* constructors are pure: `Closed` is a pure function of t; `Scanned`'s step is a pure `(s, Inputs) → (s, a)` transition — a "procedural" signal waits by *state* (a countdown in s), never by `wait`. Functions are ordinary pure lambdas; there is no separate procedure arrow — a procedure is a function whose *codomain* is `Action` (e.g. manip callbacks). `Action` is an inert first-class effect description (`wait : Float → Action`; spawn/event/manip/fork construct them; seq/par/race/loop compose them); only the control-layer scheduler executes them.
...
`t` (and the materialization axis `u`, §6) are reserved free axis names: an expression referencing `t`/`u` denotes a dyn-valued expression, exactly BDSL's movement-function model made a typing rule. Such an expression type-checks where `Dyn<T>` is expected, or where code explicitly applies it to numeric axes; it is not silently coerced into ordinary scalar slots. The `m"..."` reader macro does not change this rule: it is only alternate expression syntax, so `m"2 * ctx.t"` is a normal expression over `ctx`, while `m"2 * t"` is a dyn-valued expression. `t`/`u` (and any future axis parameters, e.g. ancestor-clock symbols) are **reserved** as free names — not bindable by `loop`/`let` — so accidental shadowing is unrepresentable; the one sanctioned binder is an explicit `fn` parameter (`(fn [t] ...)` is the literal form of a closed dyn, and inside its body a bound `t` is an ordinary number, not the axis). There are **no rate/time tags on expressions**: whether an expression is time-varying is determined by its free variables, not chosen (unlike SC's `.ar`/`.kr`, which annotates a genuine degree of freedom); the compiler infers constructor/rate, the REPL displays it, the reader greps for `t`.
...
**Streams have their own namespace: `$name`.** There may be any number of streams and none are privileged — `$player` and `$rank` are conventional names that make cards portable/combinable, nothing more. The sigil removes collision with card-local variables *structurally* (no reservation list to maintain against an open-ended host vocabulary), the host's inputs are claimed explicitly by `(from-host :name)` sites, and a card's **host-channel manifest is exactly those sites** — giving the same load-time contract check the style registry gets: a card claiming `(from-host :wind)` fails to load on a host that doesn't provide it, and a free `$name` (neither lexically bound nor card-declared) is a load error. Pattern-internal state is sigiled too: a local `(let [$x init] ...)` is a fresh private stream, so the one namespace covers the whole read-write ambient.
...
1. read-only ambient = **produced streams** (single producer, taped, readable anywhere without threading);
2. read-write ambient = **local streams** (below; pattern-scoped, shared only by explicit handle passing);
3. **scoped overrides**: `(with {$rank 0.5} body)` — dynamic *binding*, not mutation. **`with` is to channels what `in-frame` is to poses**: the same distribution law over the action tree (pushes through control combinators, lands on spawns, which capture it for their signals' lifetimes — `live` reads, evolve steps, and lowered reads alike, long after the body's evaluation), realized the same way (an extent frame on the task stack, so callees the body invokes resolve through it). The ambient frame is the special case for the "where am I" channel; `with` generalizes the mechanism. `let` cannot substitute — and post-unification the two are distinct binders, not ambiguous spellings: `(let [$x v] ...)` always *constructs* a fresh private stream, lexically shadowing the name for text the body contains; `with` never constructs — its keys are ordinary references to existing streams (a free `$name` in the map is a load error), rebinding their resolution for code the body *causes* (callees resolve channels in their own definitions; spawned signals outlive the body). The map is a map because the keys evaluate: unordered = bindings are independent, values all read the outer extent (no `let*` sequencing to misread). Settled (2026-07, scoped-channel-overrides): every stream is overridable — injected, derived, and local uniformly, with the base stream, its producer, and the replay tape untouched; nesting shadows innermost-first with extent exit as a pure scope pop (fresh override cells per extent, no restore writes); values evaluate once at form evaluation (snap), a stream-handle value aliases (reads deref to the source's current value), and `with` is a binder, not a producer form — per-tick overrides compose via `bind!`, which inside an extent acts on the override cell (attaching to the base would replace the global producer in place and outlive the extent). Overrides are ordinary card data (tree nodes; they serialize).
...
**The export surface is three orthogonal operations** (resolving the mechanism half of §13.9): **declare** — `(def $name init?)` at top level, `(let [$name init] ...)` locally; **produce** — `(bind! $name expr)` attaches the per-tick refresh producer (one per stream; rebinding replaces in place, which is how a rig overrides a stdlib default), whose expression closes over the current environment, including local streams and entity handles, with the world-query vocabulary in scope (`(count-entities q)` / `(nearest-entity q to)` over §9's manip query maps); **publish** — `(export! $name)` / `(export! $x :as $name)` registers the stream with the host (name collisions are load errors; `:as` renames). Producers refresh in attachment order (top-level binds in card order, then runtime attachments); a `nothing` result leaves the last value — including a `set!` — standing. **`(defchannel $name expr)`** is the one-line sugar for all three — `$player`, `$lives`, `$enemies`, and `$nearest-enemy` are stdlib defchannels in lib/touhou.maku, not engine code, with host inputs claimed as `(from-host :name default)` producers. Per-pilot families are library/card conventions, not engine-derived names.

**Pattern-scoped local streams** are the *internal* analogue: `(let [$name init] ...)` declares a fresh private stream per instantiation, written by `(set! $name v)` actions (deterministic store writes → scrub survives), read plainly by the control layer (it owns them, tick-synchronous), and read in signal slots via `(live $name)` — snap-by-default applies to local streams exactly as to injected ones. SC control-bus precedent. Scoping is lexical: the handle rides ordinary bindings and closure capture, and passing state to another pattern or defn is an explicit sigiled parameter — no dynamic scope exists to shadow defs, which keeps signal-slot name resolution static (defs and builtins bind at load/lowering time). Local streams are for state read *concurrently* by long-lived signals and independent loops; where gating is structural (successive stages of a loop), structure is still preferred.
...
- Constructors: closed-form (`f(t) → Pose`, with cartesian and polar point variants; `pos`/`vel`/`acc` variants are `integrate` applied 0/1/2 times — vel/acc are scans by construction, with self-state slot bindings per §3), and procedural per-tick (`evolve` directly). `vel` is the ONE stock integrator evolve over reserved `:vel-x`/`:vel-y` component columns (landed 2026-08; semantics in `openspec/specs/evolve-semantics/spec.md` — components are readable entity fields, concurrent integrators in one tree are a spawn error, `stages` segments share). `lerp`-family speed profiles in vel slots are the common Scanned case; see the F1 lint in §9.
...
- Dyns are applicable. A dyn in head position with a numeric first argument
  ...
  `u = 1`, with tangent heading. A non-numeric child keeps the frame-application
  meaning, e.g. `(d child)` composes the dyn as a frame over the child.
...
- Spawn slots (`figure`, `colliders`, `meta`) broadcast likewise; atoms lift.
...
- RNG is **counter-based** (`rand(seed, path, k)`): element k's randomness independent of evaluation order — required for array spawning, scrubbing, and rewind to coexist. Surface `(rand lo hi)` / `(rand-int lo hi)` / `(randpm1)` key implicitly off the drawing context: hierarchical task-tree keys for live draws, spawn scope + element ordinal + signal-node ordinal + site number for captures, entity key + domain + tick for rowful scratch contexts (evolve steps, field fns). DMK's unsafe-`rand` vs bullet-seeded-`brand` distinction does not exist here — all randomness is replay-safe by construction. Landed 2026-08 (`rng-spawn-order-independence`, splitmix-finalizer mix rather than Philox — the algorithm is behind the contract); the seed is host-settable and rides the session command tape. Contract detail: `openspec/specs/determinism/spec.md`.
...
- `(spawn figure meta colliders)` is the **low-level entity constructor**. It is an action (never a signal), and it returns `EntityRef` handles in the same broadcast shape as the spawned elements.
...
- Current surface: `(spawn dyn meta/projector...)` accepts merged flat field maps plus explicit collider projectors. Multiple meta maps merge per-key with later maps winning. Collider projector slots accept a projector value, a one-level list of projector values, or `nothing`; lists concatenate their projector values, while nested lists or non-projector list elements are errors. Collider projectors are explicit arguments such as `(circle-collider ...)`, `(capsule-chain-collider ...)`, inline `(collider :pose [e ctx] body...)`, or named `defcollider` projector values; dynamic collider availability is expressed with ordinary control flow in projector bodies, not dynamic spec lists. Rendering is not a spawn slot. Touhou library helpers own friendly names such as `bullet`, `shot`, `enemy`, `player`, `boss`, and `laser`.
...
- Render-affecting fields are not core vocabulary by default. A render rule or host profile may interpret fields such as `:hue`, `:scale`, `:facing`, `:opacity`, `:family`, `:color`, or `:variant`, but those meanings live in library/host config. Core only preserves typed data and the render row boundary.
- Style-like records are host/library policy. A Touhou library can define `{:family :gem :color :yellow :variant :w}` and a host renderer can intern that record for pooling; core should not require those exact axes.
...
- Triggered controls: `(par pattern (seq (wait-for (< hp 0.5)) (manip query f)))`. Event vocabulary: collisions, grazes, thresholds, host-injected events.
...
- `(manip target callback)` applies `callback` to each live target. `target` may be:
  ...
  The callback receives an entity handle/view appropriate to action construction. It may return/execute instant actions (`change-col`/`set-col`, `remat`, `cull`, `spawn`, events) or, in a compiler, lower to masked SoA writes when it is pure and slot-local.
...
- **Epoch model** (remat clock semantics): every rematerializable slot carries an epoch column; birth time is just each slot's initial epoch. `(remat bullet slot new-signal)` writes `epoch := now`; the new signal runs on `τ = t − epoch`, starting at 0 at the event tick. Initial conditions are passed explicitly — the remat call snaps what it needs and hands it to the new signal as ordinary `ir` constants — so C⁰ continuity holds by construction; C¹ is a convention of stock helpers (`remat-straight = linear(snap pos, snap vel)`). Remat is **per-slot**: a half-finished fade keeps running on its own epoch when the motion slot is swapped. A bullet's history is a list of `(epoch, signal, constants)` segments — piecewise-`Closed` bullets remain fully scrubbable, and the segment record is exactly the replay-log entry. `stages` (§3) is this same model statically scheduled. Ancestor clocks stay orthogonal: remat moves only the local epoch. Like all writes, remat applies at the next tick boundary: `(remat b spec)` queues; the exit state (`:pos`/`:vel`/`:t`) is snapped at the boundary, not at the call, and reads later in the same tick still see the old motion and fields. The spec is **partial** — either the motion-only sugar (a dyn or `(fn [exit] …)`) or a map whose reserved `:motion` key holds the motion entry and whose other keys are field writes (constants or `change-col`-style update fns), all applied atomically at the same boundary in queue order; absent slots retain. A motion remat restarts only the motion slot's clock and state — entity age `(:t e)` and dyn meta fields keep the spawn clock. Motion and update fns evaluate against defs only (closed, like every boundary write).
- **The F1 lint — no silent strengthening**: velocity constructors with closed-form-integrable integrands (constants, piecewise-affine `lerp` profiles) are `Scanned` as written but have `Closed` equivalents; the compiler never rewrites silently (a scan stays a scan — predictability) but lints with the suggested closed rewrite. This matters compositionally: one `Scanned` guide contaminates every rider by contagion (the cradle: one `vel` makes 126 petals unscrubbable; the closed rewrite restores the whole tree). Implemented 2026-08 as the Advisory-confidence checker diagnostic `motion/scanned-closed-form` — Advisory never enforces.
...
- **Filters/latches are ordinary code and fields**: once-only and skip behavior are expressed in the callback/query with ordinary fields, e.g. checking `a.grazed` before setting it. Handles expose `:pos`, `:vel`, `:t`, `:tick`, `:kind`, style axes, `:team`, and flat fields such as `:damage`. `(emit :events {:name :name :pos pos})` is the primitive event operation; prelude `(event :name)` and `(event :name pos)` are macro sugar, with a point-pose second argument supplying the event position. Reactions stay control-layer (`wait-for`, derived channels such as `$graze`/`$enemies`); the engine knows detection, not damage/graze/shot semantics.
- **Invulnerability is a column**: `iframe-until` (a tick stamp) is honored by BOTH resolve paths — a hit inside the window doesn't land (player side), a shot is *absorbed* (dies, emits `absorbed`, no hp write — boss side). `(invuln b dur)` writes it — and is *library code*, not an engine verb: `(set-col b :iframe-until (+ $tick (* dur 120)))`, a deadline computed from the `$tick` channel. The automatic post-hit window reads its duration from an `:iframes` column (seconds) when present. Being columns, windows snapshot and scrub like everything else. `(change-col b :name f)` is the primitive field write: a *functional* update queued to apply at the next tick boundary — all reads within a tick see pre-tick state (row indices stay stable), and a slot's queued updates compose in action-execution order over the pre-tick value, so concurrent increments accumulate instead of losing writes. `(set-col b :name v)` is prelude sugar for the constant update `(fn [_] v)`; last-writer-wins for plain sets is a consequence of queue order, not a rule. Update functions must be pure — they evaluate against defs only (no channels, cells, or world).
- **Shapes**: circles in the dense fast path; lasers derive capsule chains *per tick from the same curve figure the beam render rule samples* (the beam you see is the beam that hits — collider projectors and `(curve-samples ...)` read one figure, each producing its own static description per tick). Lifecycle is library code: the touhou `laser-collider` body returns no colliders during warn and sweeps the hot extent from `:warn`/`:active`/`:fill` fields while active, the beam look is a library `deftick` rule over `:render :beam`, and a library cull rule ends the beam — core knows none of it. Beams persist through hits. Heterogeneity is confined to the rare shape.
...
- **Everything writes World** (counters, events, latches, fields), so the whole gameplay layer snapshots and scrubs with the timeline; collision fact generation and tick-rule order are canonical so replays agree. Events carry positions; render rules may draw effect flashes statelessly from the event log — they replay under scrubbing for free.
...
- **The standard library is a card, shipped inside the engine.** A *bare* import name — `(import "touhou")`, no slash, no `.maku` — resolves to a library card (`@lib/touhou.maku` is its include-once key) **embedded in the engine artifact at compile time**: authored as ordinary `.maku` files (crates/core/lib/), inlined via the build, identical on every host — native, wasm, headless — with no filesystem or fetch involved, and available to in-memory sources (tests, REPL, rig strings, swap/add) as well as files. Users import the library; they don't edit it. The genre layer lives there (the spawn templates, `player`/`player-rig`, `invuln`), and the direction of travel is that anything expressible as card code moves there — the engine keeps only what card code cannot express. **The prelude is the autoimported slice of the stdlib**: expansion prepends `@lib/prelude.maku` to every top-level source (a first-line sentinel keeps re-expansion and explicit imports idempotent). It holds *language-level* sugar only — `when`/`unless` are prelude macros over `if`, and `render`/`event` are prelude macros over the core `(emit :render|:events …)` primitive; anything a genre could disagree with goes in an importable lib instead. `for` remains engine for now (its `:every`/`inf`/array-iteration semantics live in the scheduler, not in a desugar).
- **Pattern embedding is scope-disciplined, with a safe default.** `(pattern arg…)` invokes a card pattern: arguments evaluate in the *caller's* scope as ordinary `ir` values, defaults fill the rest, and instance state is **local sigiled streams** — two embeddings of the same pattern cannot share `(let [$x ...])` state, so double-embedding is safe by default. Sharing is **explicit handle passing**: a sigiled parameter (`(defpattern turret [$ammo 30] ...)`) receives the stream itself, not its snap — the caller writes `(turret $shared)` and both instances see the same slot (a defaulted sigiled param allocates a fresh private stream). `defn`s share the same way (the guide-rig-takes-`$mode` idiom) — hygiene has no cell exception left; only slot-bound `t`/`u` cross. Read-only streams flow through embedding unconditionally. Handles are ordinary values in the canonical tree — the composition-level analogue of the `shared` node (§5) — and ambient frames still do not cross (§4). Residual **[decide]**: a `:sealed` adapter blocking channel overrides (`with` landed; the mechanism is an empty barrier frame on the extent stack).
...
8. ~~`(with {channel value} body)` residual details~~ **resolved** (2026-07, scoped-channel-overrides — implemented): all channels overridable uniformly; innermost-wins shadowing over fresh per-extent cells; values snap at form evaluation with stream-handle aliasing, and per-tick override values compose via `bind!` on the override cell (see §3). Live signal-valued overrides beyond handle aliasing stay out: `live` values are dyn-typed, and blessing them would smuggle a second aliasing spelling.
9. Derived-stream vocabulary and cost model (§3). Resolved: declare/produce/publish (`def $x` / `bind!` / `export!`) with `defchannel` as the one-line sugar; instance-scoped state is a local stream the producer closes over. Core no longer owns genre streams; Touhou defines the DMK/BDSL scalar defaults, and co-op-style per-pilot families are library/card conventions.
...
12. ~~Pattern-embedding scope adapters~~ **resolved** (§10): isolated local streams by default, sharing by sigiled-parameter handle passing, arguments as caller-scope `ir` values. Remaining: the `:sealed` channel-override adapter — `with` has landed (§13.8), so this is now free-standing; the mechanism is an empty barrier frame on the extent stack.
...
| Local control streams (`let [$x]`/`set!`/`live`) | SC control buses; DMK `exec`-hvar + `whiletrue`, event-logged |
