...
12. ~~Pattern-embedding scope adapters~~ **resolved** (§10): isolated local streams by default, sharing by sigiled-parameter handle passing, arguments as caller-scope `ir` values. Remaining: the `:sealed` channel-override adapter (waits on `with`, §13.8).
