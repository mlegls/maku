...
## Purpose
The deterministic session/replay/scrub contract: what makes any tick
reachable, replays exact, and live code changes scrub-safe.
...
The world MUST expose a host-facing seed reset (`reset_rng(seed)`), and construction MUST seed by implicitly calling it with the default constant. A mid-run reset MUST be recorded as a command on the program tape and replayed at its tick during seek and re-run; replay fidelity is promised only for tape-recorded resets. The active seed MUST be carried by world snapshots so scrubbing across a reset is exact. Landed 2026-08: `ProgCmd::Seed` beside add/swap, `Session::record_seed`, host `(seed <n>)`.

#### Scenario: Seek across a mid-run reset
- **WHEN** a session records a seed reset at tick N, steps past it, and then seeks back before N and forward again
...
Implementation shape (formerly in docs/player.md): snapshots are taken
about once per second and auto-thinned to logarithmic density — recent
scrubbing stays fine-grained while the tick-0 baseline always survives.
The event log is one shared append-only structure; snapshots hold only
a cursor into it. The wire protocol carrying command-tape entries is
documented user-side in docs/player.md.
