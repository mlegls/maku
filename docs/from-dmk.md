...
(MIT, Â© Bagoum). This engine's design started from a close reading of
DMK's semantics â much of the vocabulary corresponds directly, expressed
...
| DMK | here |
|---|---|
| `CreateShot2(x, y, speed, angle, style)` | `(bullet ((pose c[x y]) (linear p[speed angle])) {:style â¦})` |
| `sync(style, rv2, s(...))` | `(bullet dyn {:style â¦})` |
| engine-owned pools / player / BEH prefabs | `(import "touhou")` â the genre layer is a library card (spawn templates, `invuln`, the stock rig) over a bare `spawn` of dyn + explicit meta |
| V2RV2 `<nx;ny:rx;ry:Î¸>` | position of `pose`/`+` written inside vs outside `rot` frames; the Î¸ slot is the `rot` itself |
| `rvelocity(v)` / `nrvelocity(v)` | `(linear v)` inside the rotation / outside it |
| `cr(r, Î¸)` / `cxy(x, y)` / `px(x)` | `p[r Î¸]` / `c[x y]` literals |
| `gsrepeat { times(30) rv2incr(<10>) }` | `((rot m"10 * iota(30)") child)` â arrays broadcast |
...
| `color({ â¦ })` + `*` wildcards in style strings | arrays in the structured style record; axes target by length, by cyclic `nth`, or by nested matrices |
...
| persistence predicate / `newtimer()` | loop bounds, or `(until pred â¦)` |
...
| `flipygt(4, _)` | `(remat b (linear c[b.vel.x (- 0 b.vel.y)]))` â the reflection written out |
| `cull(y < -2)` | `(manip (fn [b] (< b.pos.y -2)) (fn [b] (cull b)))` |
| `batch(pred, { â¦ })` | a `seq` in the callback |
| `restyleeffect(style, fx, _)` | `(set-col b :family :arrow)` / other style-field writes, plus an effect spawn if wanted |
| `sm(_, sync â¦)` | spawn directly in the callback, anchored by `((pose (pos b)) â¦)` |
...
| DMK | here |
|---|---|
| `async â¦ gcrepeat { wait(60) times(inf) }` | `(for [i inf :every t] â¦)` |
| `paction` / `saction` | `(par â¦)` / `(seq â¦)` |
| `gtrepeat { â¦ waitchild }` | `for` + `fork` for children the loop shouldn't wait on |
| `lerpt(a, b, from, to)` in movement | `(lerp a b t from to)` in a `vel`/signal slot |
| `preloop b{ hvar colorIndex = i }` | `:cols {:ci (iota n)}` â column arrays bind per element |
| `&colorIndex` in a control | `b.ci` on the bullet view |
| `start b{ hvar rootloc = loc }` | recover from the view (`tail = pos â velÂ·t`) or save a column |
| `colorf(list, &i)` | `(nth palette i)` |
| `bindLR` (lr/rl = Â±1 by loop parity) | `(- 1 (* 2 (mod k 2)))` |
| `lerpsmooth($(eiosine), a, b, t, v1, v2)` | `(lerpsmooth eiosine a b t v1 v2)` â easings are values, no lambda-conversion syntax |
| one-shot `sm` control with predicate re-summoning | a control whose spawn re-enters an earlier control's query â self-rebuilding patterns |
| `move(inf, nroffset(px(sine(8p, 2, t))))` | `(in-frame (cart m"2*sine(8, 1, t)" 0) â¦)` â a moving frame |
...
| `pattern`-level reuse / script includes | parameterized `defpattern` + `(import â¦)` |
| â (no user macros in BDSL) | `defmacro` with backtick templates |

## Pathers, lasers, subfiring (Tutorial 4)

| DMK | here |
|---|---|
| `pather(maxRemember, rememberFn, path, opts)` | `(pather window dyn)` â the trail is the hitbox |
| `laser(path, cold, hot, opts)` | `(laser (curve shape? {:u-max len â¦}) {:warn cold :active hot â¦})` â the figure is a plain curve; lifecycle is ordinary spawn meta |
| `straight(angle)` / `rotate(base, fn)` options | frame rotation: `((rot angle) (curve â¦))` / `((rot m"â¦tâ¦") (curve â¦))` |
...
| slow/filling lasers | `:fill (fill-linear warn d)` â full path telegraphs, the hitbox sweeps from the source |
...
| `sm` option (SM at the tip) | `let`-bound guide + `(in-frame guide (fork â¦))` |
| `onlaser(fn)` modifier | `(on-laser h u)` â pose with tangent heading at u |

## Difficulty and the host boundary (Tutorial 5)

| DMK | here |
|---|---|
| `dl` / `dn` / `dh` (ratio to a reference difficulty) | `$rank` â one channel, scale with `*` or soften with `pow` |
...
| `target(ang, Lplayer)` | `((aim $player) â¦)` â a frame operation |
| `Lplayer` / engine-privileged player | the player is card content: an entity with `:pilot`, deriving `$player` |
| exposing boss state to UI | `(def $boss)` plus `(bind! $boss expr)` and `(export! $boss)` for structured state — `defchannel` is the one-line sugar |

## Design philosophy (Tutorial 6)

DMK's t06 is a manifesto, not a mechanics tutorial â it introduces no
constructs. Its three arguments map at the level of principles, and each
lands somewhere specific here:

| DMK's pitch | here |
|---|---|
| movement is function-based: drop `3 + sine(4, 0.6, t)` into any velocity slot instead of falling back to manual update loops | the signal model *is* the language: every dyn/meta slot takes an expression over `t`/`u`, and closed-vs-integrated is typed (which DMK doesn't do â that's what buys scrubbing) |
| the showcase: 4 spiral bullets, `gsrepeat({ times(4), circle }, s(polar(2*t, 80*t)))` vs pages of LuaSTG/DNH update-loop code | `(bullet (circle 4 (polar m"2*t" m"80*t")))` â see `cards/translations/060_polar.maku` for a production-strength version of the same shot |
...
| extensible by "writing a C# function and putting it somewhere" â engine-language extension, engine rebuild | extension is *in-language*: `defn`/`defmacro`/lib cards. The whole touhou genre layer (spawn templates, contact rules, phases) is card code over a bare engine â a user card extends the vocabulary with no engine involvement at all |
...
| DMK | here |
|---|---|
| `pattern { } { phase â¦ phase â¦ }` | `(phases (:label opts? bodyâ¦ (finally â¦)?) â¦)` â a `(import "touhou")` macro over the `states` FSM primitive |
| the boss BEH entity | `(boss $boss dyn meta machineâ¦)` â library macro: an *enemy with a phase machine*. It owns the boss conventions: binds a map-valued `$boss` channel, holds the machine until the boss registers, binds `boss`/`boss-main` for the machine body; hp/hurtbox/triggers are ordinary meta |
| `hp(4000)` phase property | `{:hp n}` â desugars to `(until (<= (hp-of boss-main) n) body)`, reading the local boss handle; `{:until pred}` is the general gate |
| phase timeout `phase X {â¦}` | `{:timeout X}` â desugars to `(fork (seq (wait X) (goto)))`; bare goto = exit to successor |
| `root(0, 2)` phase property | `{:root c[0 2]}` â desugars to `(move-to boss-main â¦)` at the body head; the card knows its boss |
| `type(spell, "Name")` | card data: an exported cell written at state heads (or a card-level template macro) |
| `shiftphaseto(N)` (index) | `(goto :label)` â labels survive phase insertion; scoped to the innermost machine; labels are values, so routing may be computed (Markov chains) |
| the zeroeth setup phase convention | a routing state: `(:opening (goto :spell1))` â or nothing; setup is just code before the machine |
| phase-end task cancellation (token propagation) | the phase guard cancels the body's whole task subtree (`until` semantics) |
| end-of-phase item drops / cleanup | the clause's `finally` block â runs on every exit path |
| `vulnerable(false)` phase property | `(invuln boss dur)` in the previous phase's `finally` â a column both resolve paths honor |
| player invulnerability after death | automatic `iframe-until`; window duration = the `:iframes` column |
| boss HP bar / phase name on the HUD | `boss` rebinds the card's boss stream such as `$mima`; richer phase metadata is card-level state over `bind!` |

## Firing index and empty-guided fires (Tutorial 8)

| DMK | here |
|---|---|
| `p this` / `preloop b{ hvar itr = i }` | an ordinary binder: `(map (fn [k] â¦) (iota n))` per bullet, `dotimes` seq bindings per volley, `:cols` for indices the bullet carries |
| `p add` / `p mod` / `p invmod`, unpacking via `p1`/`p2`/`pm` | nothing â nested indices are separate named variables; there is no packing because the index never leaves scope |
| `bindArrow` (`axd`/`ayd`/`aixd`/`aiyd` autovars) | a formation is a function: four lines of card code mapping index â offset frame (tutorial 7) |
| `guideempty2(p, {("eloc", code(loc)), ("edir", code(dir))}, path, children)` | a level of the frame tree: `((vel path) childrenâ¦)` â the guide is an unexpressed dyn, nothing spawns |
| `dtpoffset("eloc", "edir", p, offset)` = `eloc + rotatev(edir, offset)` | frame composition â every pose carries a heading, children rotate with it automatically |
| `p` as the unique child-to-guide key | lexical nesting â the association is structural |
...
| SM riding the empty / shared guide | `let`-bound guide = the shared instance; `(spawn guide â¦)` expresses it, `(in-frame guide (fork â¦))` rides it |
...
timers), and maps to ordinary code â most rows are idioms the earlier
tutorials already teach.

| DMK | here |
|---|---|
| `bank <off>` / `bank0` ("inner repeats": shift rv2 into nonrotational coords) | frames nest â a child group composes around its parent's origin natively; rotational-vs-not is the inside/outside-`rot` choice (the V2RV2 row, Tutorial 1) |
| `bindArrow` / `bindLR` / `bindUD` | formation fns and `[1 -1]` seq bindings (Tutorial 8 table above) |
| `cancel(pred)` â checked every iteration | `(until pred loop)` â cancellation kills the loop's scope |
| `clip(pred)` â checked once at entry | `(when pred â¦)` |
| `whiletrue(pred)` â pause stepping while false | `(wait-for pred)` in the loop body |
| `unpause(sm)` â run on the resume edge | the code after `(wait-for pred)` runs exactly at resume |
| `rv2incr(<Î¸>)` | `(rot m"Î¸ * iota(n)")` â arrays broadcast |
| `spread(<Î>)` â endpoints inclusive | `(fan n step â¦)`, step = Î/(nâ1) |
| `circle` | `(circle n â¦)` |
| `color({â¦})` + wildcards | style-record arrays; axes target structurally (Tutorial 1) |
| `colorr` â reverse-direction merge | dissolves: records merge per-key, there is no string direction |
| `colorf(list, idx)` | `(nth palette expr)` |
| `delay(f)` | `(wait d)` before the loop |
| `wait(f)` | `:every` |
| `waitchild` | `for` waits its body by default; `fork` opts out |
| `root(pos)` | an explicit frame: `(in-frame :world ((pose at) â¦))` |
| `rootadjust` â root override compensating rv2 | nothing to compensate; positions compose explicitly |
| `start b{â¦}` / `preloop` / `postloop` / `end` | `let` at the head / per-iteration bindings (`dotimes` seq bindings, fn params) / fold state (`loop`/`recur`) / `finally` |
...
| `fortime(d)` â frame cap racing the count cap | `(race (wait d) (for [i n] â¦))` |
| `frv2(fn-of-i)` â offset as a function of iteration | offsets *are* expressions of the index (BoWaP: `cards/translations/130_bowap.maku`) |
| `noop` | â |
| `onlaser` | Tutorial 4 table |
| `p` / parametrization | Tutorial 8 table |
| `saoffset(SAAngle, Î¸, eq)` â summon along an equation | the offset is an expression; the `SAAngle` enum is where you place `rot`/`pose` in the tree â `banktangent` is a closed path's frame heading, free |
| `sequential` â children in sequence instead of parallel | `seq` vs `par`, always explicit |
| `sfx` / `sfxf` / `sfxif` | `(event :sfx â¦)`; index with `nth`, gate with `when` |
| `target(mode, Lplayer)` / `sltarget` | `((aim $player) â¦)` at whichever tree level you mean; the six control modes are arithmetic on explicit offsets (laser grids: the grid helpers in `cards/translations/ph_boss2_spell2.maku`) |
| `timer` / `newtimer()` â shared, resettable clocks | capture an epoch and read the world clock live: `(let [t0 $tick] â¦ m"(live($tick) - t0)/120" â¦)`; resettable = the epoch in a cell, reset by `set!` |
| `timereset` + `st` (summoning time) | `st` is fixed at spawn â pass the emitting loop's elapsed time in as a binding; resetting is rebinding |

Porting this table doubled as the Â§13.1 *ancestor clocks* audit, and
...
its own `t`. No engine operator is needed â sugar naming the idiom, if
it ever feels heavy, is lib code. The one engine change the audit
forced was a bugfix, not a feature: `(live â¦)` reads now count as
...
| secondary HP display / bottom tracker / spell stars | host reads a map-valued boss channel such as `$mima` from `(boss $mima dyn meta â¦)` |
...
| `type(non/spell, "Name")` as phase config | card-level phase metadata, commonly folded into a structured boss stream with `bind!` |
...
ordinary streams/events folded into the same map with `bind!` or a
...
| `stage` / `midboss` / `endboss` / `announce` / `dialogue` phase properties | published stage state and events, e.g. `(set! $stage {:section :dialogue})` and `(event :dialogue)`; timer freezing/UI visibility are host policy |
...
- **Repeaters vs arrays.** DMK expresses multiplicity through repeater
  ...
  broadcasting (frames Ã children) and only *sequential* firing uses a
...
- **Per-bullet mutation vs remat.** Where DMK controls often mutate
  ...
  usually a one-shot `remat` â snap state, swap motion â or a column
  written at spawn.
...
- The full translation corpus (`cards/translations/`) ports several DMK
  scripts â including a production boss spell â with notes on each
