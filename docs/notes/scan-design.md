# scan — design (settled 2026-07)

The kernel's one stateful signal constructor. Everything else in the dyn
family is either a plain function of time or lib code over `scan`.
...
- `scan` is legal in any expression position; it simply returns a dyn
  value, and that value's type is what constrains where it fits.

## scan

```
(scan init (fn [s ctx] step-expr))
```
...
- The step advances exactly once, at the tick boundary; it reads
  ...
  the settled value.
- Per-slot epochs (remat contract): rematting a slot resets its scan
  state to a fresh `init` evaluation and restarts epoch-local t/tick;
  untouched slots keep both.

## Closed vs live scans — the sampling rule

- A **closed** scan's step reads only `s`, ctx, and other dyns it closes
  over (sampled at the same t). Its fold is a pure function of
...
- A **live** scan's step reads entity cells / live channels. It is still
  a valid slot dyn — advanced by the engine on the entity's clock — but
  `(d t)` off-clock is an error (its value depends on the input trace,
  not on t). The t->T equivalence holds exactly where it can hold.
...
Note: today's `sample` special evaluates dyns against a fresh
`MotionState`, so stateful dyns sampled that way silently read
init-state. The closed/live rule replaces that accident with a
definition.

## What re-expresses over scan (lib, then AST-rewrite fast paths)

- `vel`: `(scan p0 (fn [p {:keys [dt]}] (+ p (* v dt))))`
...
- Kernel signal surface ≈ function application + `scan`.
- `sample` builtin: delete once dyn application lands.
- Push/pull split stands: `scan` is the pull-based per-slot stateful dyn
  (compilable to masked-SoA scan sites, one per slot); deftick rules +
...
## Implementation order (not yet started)

1. Dyn values callable in application position (`(d t)`, `(d t u)`);
   accept `(fn [t] ...)` in dyn slots; delete `sample`.
2. `scan` special: closed scans first (replayable), live reads erroring
   off-clock; wire to existing ScanSite state + per-slot epochs.
