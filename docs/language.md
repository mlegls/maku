...
- Each segment runs on its own epoch (per-slot epoch model, §9); the optional `(fn [exit] …)` form receives the snapped exit state of the previous segment — continuity is explicit initial-condition passing, the remat philosophy. Exit semantics (same for `remat`): **state as of the boundary tick** — `:pos` is the current-tick position, `:vel` the finite difference over the preceding tick, `:t` the segment-local age; nothing is predicted forward, and the successor anchors at the snapped pose so there is no positional jump at the boundary.
