...
- Source-level stored field names are flat and unified. There is no long-term split between arbitrary `cols` and `meta`; storage may keep numeric, symbol, handle, and pose fields in separate typed matrices, but source field access is one mechanism. Maps and lists may appear as ordinary compile-time/source values and in reserved option records such as `:style`, `:triggers`, or projector constructor arguments, but they are not retained as entity meta fields. The prototype currently keeps `:cols` as compatibility sugar for numeric fields.
