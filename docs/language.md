...
11. **`race` (general form)**: spec'd (§8) but not in the prototype. The `phases` machine IS implemented — trampoline over ordered clauses, scoped `goto` (env-carried request cell; first write per phase wins, which realizes the tree-order tie-break), `finally` on every exit path, `:timeout`/`:until`/`:root` opts, `$phase` exported as card data — with each phase's implicit race expressed as the `until` degenerate form (one synthesized guard predicate over the body scope). `:hp`/`:name`/`:type` opts are carried as card data but don't yet drive an hp pool (needs the boss-subject wiring; hp gates express as `:until (<= $boss-hp n)`). Phase-body return values as next-label (the full trampoline) also await a forcing case: routing is goto-or-clause-order.
