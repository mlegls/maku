...
- **Colliders are projector results, not entity kinds**: semantically an entity has collider projectors that evaluate `(EntityView, ProjectorContext) -> [Collider]` each tick. The returned colliders are literal shapes and layers, close to the current checked collider rows; they do not describe how to translate a figure. Most cards use projectors such as `bullet-collider`; expanding circles, warn/hot phases, disabling collision, and curve-sampled capsule chains are expressed by dynamic entity/meta inputs plus higher-order projector combinators over context such as age. Layer is universal routing metadata on every collider. An empty list or `none` collider means inert to the contact pass. Teams are ordinary meta fields only.
