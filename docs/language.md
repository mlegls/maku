...
- **Colliders are projector results, not entity kinds**: semantically an entity has collider projectors that evaluate `(Figure, Meta, EntityContext) -> [ColliderData]` each tick. Most cards use constant-shape projectors such as `bullet-collider`; expanding circles, warn/hot phases, disabling collision, and curve-sampled capsule chains are expressed by dynamic figure/meta inputs plus projector-local context such as age. Layer is universal routing metadata on every collider. An empty list or `none` collider means inert to the contact pass. Teams are ordinary meta fields only.
