...
- `defcollider` is top-level only and declares a projector function from `(Figure, MetaEnv, EntityContext)` to literal `Collider` rows. The surface form names the entity view explicitly, e.g. `(defcollider bullet-collider [e] body)`. Its body is ordinary pure code over `e` and context; `let` is the normal way to share computed values. Collider constructors are typed projector constructors: their argument records must have statically known shape, but field values may be arbitrary pure expressions over `e`/context. `EntityContext` supplies entity-local age/`t`, tick, and handle identity; projector authors use it for behavior intrinsic to the projector, such as "active until age 0.5", without allocating a public meta field. Dynamic collider data that cards query, share, export, or manipulate still belongs in meta. Empty collision and disappearing collision are ordinary projector results; `none` remains a collider variant, not option/maybe semantics, so projector combinators can disable a branch without changing type.
- `defrenderer` declares a projector function from `(Figure, MetaEnv, EntityContext)` to render rows. A render row is kind-indexed: the kind (`:sprite`, `:polyline`, `:mesh`, etc.) selects a registered schema, and the remaining fields are checked against that schema. Core does not prescribe kind meanings beyond registry, manifest, extraction order, and typed transport. Render-affecting values such as style, scale, opacity, palette, or batch keys live in meta if cards or hosts need to observe/manipulate them.
...
- Projectors are ordinary composable higher-order values at the authoring level. A Touhou bullet can use `(defcollider bullet-collider [e] (+ (hit-collider e) (graze-collider e)))`; a laser can use a projector that samples a curve figure into capsule chains, with width/active state/layer read from meta. Time combinators choose between projector functions, e.g. "if age < x then `collider-a(figure, meta, ctx)` else `collider-b(figure, meta, ctx)`". A namespace adaptation combinator rebinds the meta environment seen by an imported projector, e.g. "evaluate `collider` with `env.old-name = env.new-name`" or with `env = env.collision`. This preserves the expressive power of direct dynamic collider row lists while keeping non-positional dynamic state in one place.
...
- **Colliders are projector results, not entity kinds**: semantically an entity has collider projectors that evaluate `(Figure, MetaEnv, EntityContext) -> [Collider]` each tick. The returned colliders are literal shapes and layers, close to the current checked collider rows; they do not describe how to translate a figure. Most cards use projectors such as `bullet-collider`; expanding circles, warn/hot phases, disabling collision, and curve-sampled capsule chains are expressed by dynamic figure/meta inputs plus higher-order projector combinators over context such as age. Layer is universal routing metadata on every collider. An empty list or `none` collider means inert to the contact pass. Teams are ordinary meta fields only.
