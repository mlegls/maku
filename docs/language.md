...
- Projectors are ordinary composable higher-order values at the authoring level. A Touhou bullet can use `(defcollider bullet-collider (+ hit-collider graze-collider))`; a laser can use a projector that samples a curve figure into capsule chains, with width/active state/layer read from meta. Time combinators choose between projector functions, e.g. "if age < x then `collider-a(figure, meta, ctx)` else `collider-b(figure, meta, ctx)`". A namespace adaptation combinator rebinds the meta environment seen by an imported projector, e.g. "evaluate `collider` with `env.old-name = env.new-name`" or with `env = env.collision`. This preserves the expressive power of direct dynamic collider row lists while keeping non-positional dynamic state in one place.
