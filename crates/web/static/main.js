...
import { createCanvas2DRenderer } from './canvas-renderer.js';
...
let canvasRenderer;
let runtimeIdentity;
const pendingExternalTextures = new Map();

export function registerExternalRenderTexture(key, source) {
  pendingExternalTextures.set(key, source);
  canvasRenderer?.registerExternalTexture(key, source);
}
...
async function boot() {
  renderLists();
  await initMaku();
  runtimeIdentity = releaseIdentity();
  await loadSources();
  maku = createMaku();
  canvasRenderer = createCanvas2DRenderer({
    maku, context: ctx, worldToCanvas: [sx, sy], pixelsPerUnit: PPU,
  });
  for (const [key, source] of pendingExternalTextures) {
    canvasRenderer.registerExternalTexture(key, source);
  }
  await canvasRenderer.resolveManifest();
...
function updateHud() {
  ...
  const renderDiagnostics = maku.render_diagnostics();
  els.status.textContent = maku.status()
    + (runtimeIdentity ? `\nMaku ${runtimeIdentity.makuVersion} · ABI ${runtimeIdentity.frameAbiVersion} · ${runtimeIdentity.sourceRevision}` : '')
    + (renderDiagnostics ? `\nrender profile: ${renderDiagnostics.replaceAll('\n', '  ')}` : '')
